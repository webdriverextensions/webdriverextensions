...
import com.github.webdriverextensions.internal.utils.HtmlTag;
import com.github.webdriverextensions.internal.utils.NumberUtils;
import com.github.webdriverextensions.internal.utils.WebElementUtils;
...
public class Bot {
    ...
    public static WebElement findElement(HtmlTag tag, String locator, String identifier) {
        return WebElementUtils.findWebElement(tag, locator, identifier);
    }
    ...
    public static WebDriver driverDefault() {
        return WebDriverExtensionsContext.getDriverDefault();
    }

    public static void dragAnddrop(WebElement fromWebElement, WebElement toWebElement) {
        Actions actionBuilder = new Actions(driver());
        actionBuilder.dragAndDrop(fromWebElement, toWebElement).perform();
    }

    public static void mouseMoveToElement(String linkText) {
        new Actions(driver()).moveToElement(driver().findElement(By.linkText(linkText))).build().perform();
    }

    public static void selectMenu(String menuNames) {
        if (menuNames.contains(";")) {
            String[] menusItems = menuNames.split(";");
            for (int iterator = 0; iterator < menusItems.length; iterator++) {
                new Actions(driver()).moveToElement(driver().findElement(By.linkText(menusItems[iterator]))).build()
                        .perform();
                reporter().pass("And Then Hover On the Menu Item <b>" + menusItems[iterator] + "</b>");
                waitFor(2);
            }
            driver().findElement(By.linkText(menusItems[menusItems.length - 1])).click();
            reporter().pass("And Clicked the Menu Item <b>" + menusItems[menusItems.length - 1] + "</b>");
        }
    }

    public static void selectMenu(String menuNames, HtmlTag menuTag, boolean jsMode) {
        try {
            if (menuNames.contains(";")) {
                String[] menusItems = menuNames.split(";");
                for (int iterator = 0; iterator <= menusItems.length; iterator++) {
                    WebElement menuElement = findElement(menuTag, null, menusItems[iterator]);
                    hoverClick(menuElement);
                    reporter().pass(
                            "And Then Invoked JavaScript Click On the Menu Item <b>" + menusItems[iterator] + "</b>");
                    waitFor(2);
                }
            }
        } catch (Exception e) {
    ...
    public static void rightClick(WebElement webElement) {
        Actions actionBuilder = new Actions(driver());
        actionBuilder.contextClick(webElement).perform();
    }

    public static void rightClick() {
        Actions actionBuilder = new Actions(driver());
        actionBuilder.contextClick().perform();
    }

    public static void hoverClick(WebElement webElement) {
        try {
            String mouseOverScript = "if(document.createEvent){var evObj = document.createEvent('MouseEvents');evObj.initEvent('mouseover',true, false); arguments[0].dispatchEvent(evObj);} else if(document.createEventObject) { arguments[0].fireEvent('onmouseover');}";
            ((JavascriptExecutor) Bot.driver()).executeScript(mouseOverScript, webElement);
            Thread.sleep(1000);
            ((JavascriptExecutor) Bot.driver()).executeScript("arguments[0].click();", webElement);
        } catch (Exception e) {
        }
    }
    ...
    public static void click(WebElement webElement) {
        ...
        try {
            ...
            waitForPageUntilToComplete();
            if (elementTag.equalsIgnoreCase("a") || elementTag.equalsIgnoreCase("button")) {
                elementInfo = BotUtils.htmlOf(webElement);
            } else {
                elementInfo = WebElementUtils.getElementText(webElement);
            }
            if (elementInfo != null) {
                detailedMessage = "And Clicks on the " + elementInfo;
            }
            reporter().pass(detailedMessage);
            webElement.click();
        } catch (Exception e) {
        }
    }

    public static void click(WebElement element, boolean safemode, int retry) {
        boolean isElementStaled = false;
        int attempts = 0;
        while (attempts < retry) {
            try {
                isElementStaled = element.isDisplayed();
                if (isElementStaled)
                    break;
                waitFor(5);
    ...
    public static void click(WebElement webElement, boolean jsclick) {
        String detailedMessage = null;
        String elementInfo = null;
        try {
            String elementTag = webElement.getTagName();
            ...
            executeJavascript("$('arguments[0]').hover();", webElement);
            executeJavascript("arguments[0].click();", webElement);
        } catch (Exception e) {
        }
    }

    public static void waitForPageUntilToComplete() {
        try {
            String readyState = null;
            Long retry = WebDriverRunner.TimeOut;
            JavascriptExecutor jsExecutor = (JavascriptExecutor) driver();
            String readyStateScript = "return document.readyState";
            readyState = (String) jsExecutor.executeScript(readyStateScript);
            while (readyState == "complete") {
                readyState = (String) jsExecutor.executeScript(readyStateScript);
                reporter().debug("And Then Page document ready state still <b>" + readyState + "</b>.");
                waitFor(2);
                retry--;
                if (retry < 0)
                    break;
    ...
    public static void type(String text, WebElement webElement) {
        ...
        String detailedMessage = "And Enters <b>" + quote(text) + "</b> on the "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void deselect(WebElement webElement) {
        if (isSelected(webElement)) {
            String detailedMessage = "And UnChecked the " + BotUtils.htmlOf(webElement);
            reporter().pass(detailedMessage);
            webElement.click();
        }
    }
    ...
    public static void selectOption(String text, WebElement webElement) {
        ...
        String detailedMessage = "And Select the text <b>" + quote(text) + " </b> from "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void deselectOption(String text, WebElement webElement) {
        ...
        String detailMessage = "And UnSelect the text <b>" + quote(text) + " from </b>"
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailMessage);
    }
    ...
    public static void deselectAllOptions(WebElement webElement) {
        ...
        String detailedMessage = "And Deselected all the options from <b>"
                + quote(WebElementUtils.getElementText(webElement)) + "</b>";
        reporter().pass(detailedMessage);
    }

    public static void selectOptionWithValue(String value, WebElement webElement) {
        new Select(webElement).selectByValue(value);
        String detailedMessage = "And Select the option with the value <b>" + quote(value) + " </b> from "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void deselectOptionWithValue(String value, WebElement webElement) {
        ...
        String detailedMessage = "And UnSelect the option with the value <b>" + quote(value) + " </b> from "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void selectOptionWithIndex(int index, WebElement webElement) {
        ...
        String detailedMessage = "And Select the option with the index <b>" + quote(index) + " </b> from "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void deselectOptionWithIndex(int index, WebElement webElement) {
        new Select(webElement).selectByIndex(index);
        String detailedMessage = "And UnSelect the option with index <b>" + quote(index) + " </b> from "
                + WebElementUtils.getElementText(webElement);
        reporter().pass(detailedMessage);
    }
    ...
    public static void open(String url) {
        ...
        String detailMessage = "Browser launched with the address " + "<a>" + url + "</a>";
        reporter().pass(detailMessage);
    }
    ...
    /* Take Screenshot */
    /**
     * Takes a screenshot and saves it as a file in a directory called
     * {@code screenshots}. A custom path to the directory where the screenshots are
     * saved can be set by setting the system property
     * {@code webdriverextensions.screenshotspath}.
     *
     * @param fileName
     *            the filename of the screenshot file without the file extension
     */
    ...
    public static String takeScreenShot() {
        String fileName = getUniqueTimeStamp() + ".png";
        String absolutePath = System.getProperty("user.dir") + "\\reports\\screenshots\\" + fileName;
        String relativePath = "screenshots\\" + fileName;
        File screenshotFile = ((TakesScreenshot) driver()).getScreenshotAs(OutputType.FILE);
        try {
            copyFile(screenshotFile, new File(absolutePath));
        } catch (IOException e) {
        }
        return relativePath;
    }

    public static String getScreenShotOfElement(WebElement webElement) throws IOException {
        String fileName = getUniqueTimeStamp() + ".png";
        String absolutePath = System.getProperty("user.dir") + "\\reports\\screenshots\\" + fileName;
        String relativePath = "screenshots\\" + fileName;
        File screenshotFile = ((TakesScreenshot) driver()).getScreenshotAs(OutputType.FILE);
        BufferedImage fullScreenCapture = takeDesktopScreenShot();
        Point topLeft = webElement.getLocation();
        Dimension elementDims = webElement.getSize();
    ...
    public static String getUniqueTimeStamp() {
        DateFormat df = new SimpleDateFormat("HHmmssS");
        return df.format(new Date());
    }

    private static BufferedImage takeDesktopScreenShot() throws IOException {
    ...
    public static boolean isDisplayed(WebElement webElement, long secondsToWait) {
        try {
            WebElement foundWebElement = new WebDriverWait(driver(), secondsToWait)
    ...
    public static void assertIsNotDisplayed(WebElement webElement, long secondsToWait) {
        if (isDisplayed(webElement, secondsToWait)) {
            throw new WebAssertionError("Element is displayed within " + secondsToWait + " seconds when it shouldn't",
                    webElement);
        }
    }
     ...
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing an attribute
     *            to return
     ...
     *
     * no input in html
     * idIn(input) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing an id
     *            attribute
     * @return the id attribute
     */
     ...
     *
     * no input in html
     * nameIn(input) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a name
     *            attribute
     ...
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a class
     *            attribute
    ...
    public static void assertHasNotClass(String className, WebElement webElement) {
        if (hasClass(className, webElement)) {
            throw new WebAssertionError("Element has class " + quote(className.trim()) + " when it shouldn't",
                    webElement);
        }
    }
    ...
    public static void assertHasClassContaining(String searchText, WebElement webElement) {
        if (hasNotClassContaining(searchText, webElement)) {
            throw new WebAssertionError("Element does not have class containing text " + quote(searchText.trim()),
                    webElement);
        }
    }
    ...
    public static void assertHasNotClassContaining(String searchText, WebElement webElement) {
        if (hasClassContaining(searchText, webElement)) {
            throw new WebAssertionError(
                    "Element has class containing text " + quote(searchText.trim()) + " when it shouldn't", webElement);
        }
    }
    ...
    public static void assertHasClassStartingWith(String prefix, WebElement webElement) {
        if (hasNotClassStartingWith(prefix, webElement)) {
            throw new WebAssertionError("Element does not have class containing prefix " + quote(prefix.trim()),
                    webElement);
        }
    }
    ...
    public static void assertHasNotClassStartingWith(String prefix, WebElement webElement) {
        if (hasClassStartingWith(prefix, webElement)) {
            throw new WebAssertionError(
                    "Element has class containing prefix " + quote(prefix.trim()) + " when it shouldn't", webElement);
        }
    }
    ...
    public static void assertHasClassEndingWith(String suffix, WebElement webElement) {
        if (hasNotClassEndingWith(suffix, webElement)) {
            throw new WebAssertionError("Element does not have class containing suffix " + quote(suffix.trim()),
                    webElement);
        }
    }
    ...
    public static void assertHasNotClassEndingWith(String suffix, WebElement webElement) {
        if (hasClassEndingWith(suffix, webElement)) {
            throw new WebAssertionError(
                    "Element has class containing suffix " + quote(suffix.trim()) + " when it shouldn't", webElement);
        }
    }

    public static void assertHasClassMatching(String regExp, WebElement webElement) {
        if (hasNotClassMatching(regExp, webElement)) {
            throw new WebAssertionError("Element does not have class matching regExp " + quote(regExp.trim()),
                    webElement);
        }
    }
    ...
    public static void assertHasNotClassMatching(String regExp, WebElement webElement) {
        if (hasClassMatching(regExp, webElement)) {
            throw new WebAssertionError(
                    "Element has class matching regExp " + quote(regExp.trim()) + " when it shouldn't", webElement);
        }
    }
     ...
     *
     * no input in html
     * valueIn(input) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a value
     *            attribute
     ...
     *
     * <p>
     * If the value attribute in the {@link org.openqa.selenium.WebElement} does not
     * contain a valid number a {@code java.util.NumberFormatException} will be
     * thrown. If the {@link org.openqa.selenium.WebElement} does not exist in the
     * html a {@code org.openqa.selenium.NoSuchElementException} will be thrown.
     * </p>
     *
     * <p>
     * <b>Examples:</b>
     ...
     *
     * no input in html
     * valueInAsNumber(input) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a value
     *            attribute with a number
     ...
     *
     * no input in html
     * hrefIn(input) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a href
     *            attribute
     ...
     *
     * no span in html
     * textIn(span) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing the text
     * @return the visible text
     */
    public static String textIn(WebElement webElement) {
        // Text is trimmed to normalize behavior since Chrome and PhantomJS driver
        // incorrectly returns spaces around the text (Not according the the WebElement
        // tetText docs), see bug report
        // https://github.com/seleniumhq/selenium-google-code-issue-archive/issues/7473
    ...
    public static void assertHasNotText(WebElement webElement) {
        if (hasText(webElement)) {
            throw new WebAssertionError("Element has text " + quote(textIn(webElement)) + " when it shouldn't",
                    webElement);
        }
    }
     ...
     *
     * <p>
     * If the text in the {@link org.openqa.selenium.WebElement} does not contain a
     * valid number a {@code java.util.NumberFormatException} will be thrown. If the
     * {@link org.openqa.selenium.WebElement} does not exist in the html a
     * {@code org.openqa.selenium.NoSuchElementException} will be thrown.
     * </p>
     *
     * <p>
     ...
     *
     * no span in html
     * textInAsNumber(span) throws org.openqa.selenium.NoSuchElementException}
     * </pre>
     * </p>
     *
     * @param webElement
     *            the {@link org.openqa.selenium.WebElement} containing a text with
     *            a number
    ...
    public static void assertHasNotOptionWithValue(String value, WebElement webElement) {
        if (hasOptionWithValue(value, webElement)) {
            throw new WebAssertionError("Element has option with value " + quote(value.trim()) + " when it shouldn't",
                    webElement);
        }
    }
    ...
    public static void assertHasNotOptionWithIndex(int index, WebElement webElement) {
        if (hasOptionWithIndex(index, webElement)) {
            throw new WebAssertionError("Element has option with index " + quote(index) + " when it shouldn't",
                    webElement);
        }
    }
